    ...
    <header className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-3">
        <button
          ...
          aria-label={`Toggle practice mode, currently ${isRoadMode ? 'Road Mode' : 'Home Mode'}`}
          aria-pressed={!!isRoadMode}
        ...
        >
          ...
          <span>{isRoadMode ? 'ROAD MODE' : 'HOME MODE'}</span>
        </button>
      </div>
    </header>
  );
  ...
  return (
    <div className="hidden md:block mt-8 p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-2">
      <div className="flex justify-between items-center text-slate-400">
        ...
        <span className="font-bold text-amber-400">🔥 {academicStreak}d</span>
      </div>
      <div className="flex justify-between items-center text-slate-400">
        <span>Piano Practice</span>
        <span className="font-bold text-amber-400">🎹 {pianoStreak}d</span>
      </div>
    </div>
  );
}
...
function SidebarNav({
  ...
  return (
    <nav className="w-full md:w-64 bg-slate-900 border-r border-slate-800 p-4 shrink-0" aria-label="Main Navigation">
      ...
      {store && (
        <StreakInfo
          academicStreak={store.academicStreak}
          pianoStreak={store.pianoStreak}
        />
      )}
    </nav>
  );
...
export default function LayoutWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans">
      <Header isRoadMode={store?.isRoadMode} onToggleRoadMode={toggleRoadMode} />

      {/* Main Body */}
      <div className="flex-1 flex flex-col md:flex-row">
        <SidebarNav pathname={pathname} store={store} />
