...
const getKeyLabel = (midi: number, labelMode: KeyboardLabelMode) => {
  const pc = ((midi % 12) + 12) % 12;
  switch (labelMode) {
...
function PianoKey({ midi, startMidi, isActive, labelMode, onClick }: PianoKeyProps) {
  ...
  const label = getKeyLabel(midi, labelMode);
